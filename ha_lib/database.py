...
def get_last_value(rx, tx):
    # This function will receive the total amount of network usage from the database.

    # Then we check if this module is enabled and return the received and send bytes when it is not enabled.
    if not processor.config.getboolean('DATABASE', 'enabled'):
        return rx, tx
...
def get_start_value():
    ...
    sql = 'SELECT RECEIVED,SEND FROM RECORDS WHERE TIMESTAMP = (SELECT MIN(TIMESTAMP)  FROM RECORDS);'
    output = db.execute(sql).fetchone()

    # Now we check if there is anything added today, and return 2 zeros when it's not
    if output is None:
        return 0, 0
...
async def loop():
    # This function makes the database more efficient by deleting unused data it makes a daily and a monthly logger.
    # It also makes the counter reset every month

    # First of all we need to check if the database is enabled cancel this function when it's not
    if not processor.config.getboolean('DATABASE', 'enabled'):
        return

    # Receive the interval at what we should check the date and check for updates
    ...
    while True:
        # First we wait for our interval
        await asyncio.sleep(interval)

        # After that we need to see when the first row was added today and the first row added to the daily table.
